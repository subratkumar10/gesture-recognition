...
def create_folder_wise_classes():
    ...
    for x,y in zip(raw_angle_files_1, raw_angle_files_2):
        df_distance = pd.read_csv(x) # 80 columns
        df_angle = pd.read_csv(y)
        df_angle = df_angle.iloc[:,2:-2] # 14 columns

        distance = df_distance.iloc[:,0:80] # 80 columns
        area = df_distance.iloc[:,80:97]    # 17 columns
        centroids = df_distance.iloc[:,97:148] # 51 columns
        target=df_distance['148']
        ...
        df12 = pd.concat([distance,df_angle], axis=1,ignore_index=False) #94 features
        # df12=AutoEncoder(df12)
        len1=df12.shape[1]
