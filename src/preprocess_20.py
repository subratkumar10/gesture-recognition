...
def create_folder_wise_classes():
    raw_angle_files_1 = glob(os.path.join(config.INPUT_DISTANCES20, "*", "*.csv"))
    raw_angle_files_2 = glob(os.path.join(config.INPUT_RAW20, "*","ANGLES", "*.csv"))
    ...
    for x,y in zip(raw_angle_files_1, raw_angle_files_2):
        ...
        column_names=df_angle.columns

        distance = df_distance.iloc[:,0:101] # 101 columns
        area = df_distance.iloc[:,102:119]    # 17 columns
        centroids = df_distance.iloc[:,119:170] # 51 columns
        target=df_distance['target']
        ...
        # len1=df_angle.shape[1]
        # l1=list(range(1,len1+1))
        # l2=[]
        # for i in l1:
        #     i=str(i)
        #     l2.append(i)
        ...
        df_angle.columns=column_names
        # print(df_angle)
        
        df12 = pd.concat([distance,centroids,df_angle], axis=1,ignore_index=False) #166 features
        print(df12)
        # len1=df12.shape[1]
    ...
    # angle_df.to_csv(angles_save_path,index=None,header=None)
        ...
        save_dir = os.path.join(config.INPUT_PREPROCESSED20, gesture)
