...
def create_folder_wise_classes():
    ...
    for x,y in zip(raw_angle_files_1, raw_angle_files_2):
        df_distance = pd.read_csv(x,header=None) # 90 columns
        df_angle = pd.read_csv(y,header=None)
        df_angle = df_angle.iloc[:,2:-1] # 14 columns

        distance = df_distance.iloc[:,0:39] # 39 columns
        area = df_distance.iloc[:,39:56]    # 17 columns
        centroids = df_distance.iloc[:,56:] # 34 columns
        ...
        df12 = pd.concat([distance, df_angle], axis=1, ignore_index=True) # -> 53 columns
